<template>
  <div>
    ...
    <el-dialog title="档案删除" :visible="editwinshow" width="60%" :append-to-body="true">
      <el-table
        :data="tableData"
        stripe
        style="width: 100%">
        ...
        <el-table-column label="删除">
          <template slot-scope="scope">
            <el-button type="text" @click="deletecs(scope.row.id)">删除档案</el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    export default {
      ...
      methods: {
        ...
        onSubmit(){
          ...
          this.$axios.post("querydelete",params).then(response=>{
        ...
        deletecs(id){
          this.$confirm('此操作将删除该商品档案, 是否继续?', '提示', {
            ...
            var params = new URLSearchParams();
            params.append("id",id);
            this.$axios.post("deletecs",params).then(()=>{
              this.$message({
                type: 'success',
                message: '删除成功!'
              })
